var App = function () {
    var o = false; var n = false; var l = false; var b = [];
    ...
    var v = function () {
        ...
        $(".toggle-top-left-menu").click(function (B) {
            B.preventDefault(); $(".navbar-left.navbar-left-responsive").slideToggle(200)
        });
        var A = function () {
            $(".crumbs .crumb-buttons > li").removeClass("first");
            $(".crumbs .crumb-buttons > li:visible:first").addClass("first");
            if ($("body").hasClass("nav-open")) { $("body").toggleClass("nav-open") }
    ...
    var q = function () {
        ...
        var A = $(".header");
        var C = A.outerHeight();
        var x = $(document).height(); var z = $(window).height();
        var y = x - z; if (y <= C) {
            var B = x - y
        } else { var B = x }
        B = B - C;
        var x = $(document).height(); $("body").height(B)
    };
    var t = function () {
        q(); if ($(".header").hasClass("navbar-fixed-top")) { $("#container").addClass("fixed-header") }
    };
    var u = function () {
        var x = g(r, 30); $(window).resize(x)
    };
    var r = function () {
        q(); if ($.fn.dataTable) {
    ...
    var g = function (B, E, y) {
        var D, z, A, C, x;
        return function () {
            A = this; z = arguments; C = new Date();
            var G = function () {
                var H = (new Date()) - C;
                if (H < E) { D = setTimeout(G, E - H) } else { D = null; if (!y) { x = B.apply(A, z) } }
            };
            var F = y && !D; if (!D) { D = setTimeout(G, E) } if (F) { x = B.apply(A, z) } return x
        }
    }; var f = function () {
        if ($(window).width() <= 767) {
            $("body").on("movestart"
                , function (y) {
                    if ((y.distX > y.distY && y.distX < -y.distY) || (y.distX < y.distY && y.distX > -y.distY)) { y.preventDefault() }
                    var x = $(y.target).parents("#project-switcher");
                    //console.log(y);
                    //console.log(x);
                    //console.log(x.context.tagName);我修改的阻止事件冒泡
                    if (x.length || x.context.tagName == 'TH' || x.context.tagName == 'TD') { y.preventDefault() }
                }).on("swipeleft"
                , function (x) { $("body").toggleClass("nav-open") }).on("swiperight", function (x) { $("body").toggleClass("nav-open") })
        }
    };
    var d = function () {
        ...
        if (/Android|webOS|iPhone|iPad|iPod|BlackBerry/i.test(navigator.userAgent) && x == false) {
            ...
            if ($("#sidebar").hasClass("sidebar-fixed") || $(window).width() <= 767) {
                if (x && !$("#sidebar").hasClass("sidebar-fixed-responsive")) {
                    var y = 100; $("#sidebar").attr("style", "position: absolute !important;");
                    if ($(window).width() > 979) { $("#sidebar").css("margin-top", "-52px") }
                    if ($(window).width() <= 767) {
                        $("#sidebar").css("margin-left", "-250px").css("margin-top", "-52px")
                    }
                } else { var y = 7; $("#sidebar-content").slimscroll({ height: "100%", wheelStep: y }) }
            }
        }
    };
    var e = function () {
        ...
        $(".widget .toolbar .widget-collapse").click(
            function () {
                ...
                if (A.hasClass("widget-closed")) {
                    ...
                    x.slideDown(200, function () { A.removeClass("widget-closed") });
                    z.slideDown(200); y.slideDown(200)
                } else {
                    $(this).children("i").removeClass("icon-angle-down").addClass("icon-angle-up");
                    x.slideUp(200, function () { A.addClass("widget-closed") });
                    z.slideUp(200); y.slideUp(200)
                }
            })
    };
    var k = function () {
        ...
        $(".datatable.table-checkable").bind("draw"
            , function () {
                ...
                var x = false;
                if (y == A && y != 0) { x = true } else { x = false }
                z.prop("checked", x); if (z.hasClass("uniform")) { $.uniform.update(z) }
            })
    };
    var j = function () {
        var y = function (z) {
            $(z).each(function () {
                ...
                if (A.height() > B.height()) { B.css("min-height", A.height()) }
            })
        };
    ...
    var p = function () {
        ...
        $(".project-switcher-btn").click(function (z) {
            z.preventDefault(); x(this); $(this).parent().toggleClass("open");
            var A = y(this);
            $(A).slideToggle(200, function () { $(this).toggleClass("open") })
        }); $("body").click(function (A) {
            var z = A.target.className.split(" ");
            if ($.inArray("project-switcher", z) == -1 && $.inArray("project-switcher-btn", z) == -1 && $(A.target).parents().index($(".project-switcher")) == -1 && $(A.target).parents(".project-switcher-btn").length == 0) { x() }
        ...
        var x = function (z) {
            $(".project-switcher").each(function () {
                var A = $(this); if (A.is(":visible")) {
                    var B = y(z); if (B != ("#" + A.attr("id"))) {
        ...
        var y = function (z) {
            ...
            if (typeof A == "undefined") { A = "#project-switcher" } return A
        }
    }; var c = function () {
        $(".project-switcher").each(function () {
            var y = $(this); y.css("position", "absolute").css("margin-top", "-1000px").show();
            var x = 0; $("ul li", this).each(function () { x += $(this).outerWidth(true) + 15 });
            y.css("position", "relative").css("margin-top", "0").hide(); $("ul", this).width(x)
        })
    }; return {
        init:
            function () {
        ...
        , getLayoutColorCode:
            function (x) { if (m[x]) { return m[x] } else { return "" } }
        , blockUI: function (x, y) {
            var x = $(x); x.block({
                ...
                , css: {
                    top: "10%", border: "none"
                    , padding: "2px", backgroundColor: "none"
                }
                , overlayCSS: { backgroundColor: "#000", opacity: 0.05, cursor: "wait" }
            })
        }
        , unblockUI: function (x) { $(x).unblock({ onUnblock: function () { $(x).removeAttr("style") } }) }
    }
}();